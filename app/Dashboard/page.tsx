  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <nav className="flex justify-between items-center px-8 py-5 border-b border-zinc-900">
        <span className="font-mono text-sm tracking-widest text-zinc-400">RZLT</span>
        <span className="font-mono text-xs text-zinc-600">{profile.username}</span>
      </nav>
      ...
      <div className="max-w-lg mx-auto px-6 py-12">
        ...
        {!experimentComplete && (
          <div className="flex flex-col gap-4 mb-10">
            {/* Pre-session */}
            <div className={`border p-6 ${preToday ? 'border-zinc-800' : 'border-white'}`}>
              <div className="flex justify-between items-start mb-3">
                <div>
                  <p className="font-mono text-xs text-zinc-500 tracking-wider mb-1">
                    PRE-SESSION
                  </p>
                  <p className="font-bold">Before you play</p>
                </div>
                {preToday && (
              ...
              <p className="text-zinc-500 text-sm mb-4">
                Energy, stress, focus, confidence. Takes 60 seconds.
              </p>
              {!preToday ? (
                ...
                <p className="text-zinc-600 text-xs font-mono">
                  Completed at {new Date(preToday.completed_at!).toLocaleTimeString()}
                </p>
              )}
            </div>
            ...
            <div className={`border p-6 ${
              ...
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <p className="font-bold">After you play</p>
                </div>
                {postToday && (
                  <span className="font-mono text-xs text-zinc-500 border border-zinc-800 px-2 py-1">
                    DONE
              ...
              <p className="text-zinc-500 text-sm mb-4">
                Results, decisions, emotional state. Takes 60 seconds.
              </p>
              {preToday && !postToday ? (
                ...
                <p className="text-zinc-600 text-xs font-mono">
                  Completed at {new Date(postToday.completed_at!).toLocaleTimeString()}
                </p>
              ) : (
        ...
        <div className="grid grid-cols-3 gap-1 border border-zinc-900">
          {[
            { label: 'DAY', value: experimentComplete ? '✓' : `${experimentDay}/7` },
            { label: 'PRE CHECK-INS', value: totalPre ?? 0 },
            { label: 'POST CHECK-INS', value: totalPost ?? 0 },
          ].map(stat => (
            <div key={stat.label} className="bg-zinc-950 p-4 text-center">
              <p className="font-mono text-xs text-zinc-600 mb-2">{stat.label}</p>
