    ...
    1: (
      <div className="flex flex-col gap-3">
        {[
          ...
          <OptionButton key={value} label={label}
      ...
      <div className="w-full max-w-md">
        <div className="mb-8">
          <div className="flex justify-between items-center mb-3">
            <span className="font-mono text-xs text-zinc-600">BASELINE ASSESSMENT</span>
            <span className="font-mono text-xs text-zinc-600">{step} / {TOTAL_STEPS}</span>
          </div>
          <Progress value={(step / TOTAL_STEPS) * 100} className="h-px bg-zinc-900" />
        </div>

        <h2 className="text-xl font-bold mb-8 leading-snug">{questions[step]}</h2>
        ...
        <div className="flex gap-3 mt-8">
          ...
          >
            {loading ? 'Saving...' : step < TOTAL_STEPS ? 'Continue →' : 'Start experiment →'}
          </Button>
        </div>
      </div>
    </main>
